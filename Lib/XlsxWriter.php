...
class XlsxWriter
{
    ...
    protected function buildWorkbookXML()
    {
        ...
        foreach ($this->sheets as $sheet_name => $sheet) {
            if ($sheet->auto_filter) {
                $sheetname = self::sanitize_sheetname($sheet->sheetname);
                $workbook_xml .= '<definedName name="_xlnm._FilterDatabase" localSheetId="' . $i . '" hidden="1">\'' . self::xmlspecialchars(
                        $sheetname
                    ) . '\'!$A$1:' . self::xlsCell(
                        $sheet->row_count - 1,
    ...
    public static function xmlspecialchars($val)
    {
        ...
        static $goodchars = "                              ";
        return strtr(
            htmlspecialchars((string)$val, ENT_QUOTES | ENT_XML1 | ENT_SUBSTITUTE),
            $badchars,
            $goodchars
